...
export function MusicPlayer() {
  ...
  useEffect(() => {
    if (scriptStatus !== "ready" || !currentSong || !iframeRef.current) return;
  ...
  return (
    <div className="w-full bg-white/5 border border-primary/25 rounded-2xl p-4 md:p-5 backdrop-blur-md mt-2 flex flex-col gap-3.5 relative overflow-hidden">
      ...
      <iframe
        key={currentSong.id}
        ref={iframeRef}
        src={`https://w.soundcloud.com/player/?url=${encodeURIComponent(currentSong.soundcloudUrl)}&color=%23000000&auto_play=true&hide_related=false&show_comments=false&show_user=true&show_reposts=false&show_teaser=false&visual=true`}
      ...
      <div className="flex items-center justify-center gap-6">
        <button
          onClick={goToPrev}
          disabled={!isReady || shuffled.length < 2}
